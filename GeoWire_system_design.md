...
| # | 원칙 | 의미 |
|---|---|---|
| P1 | **Time-to-Wow < 5분** | API 키·DB·회원가입 없이 `npx geowire`만으로 실제 검색이 동작해야 한다 |
...
- **모든 인터페이스(MCP/REST/SDK/CLI)는 얇은 어댑터**이고, 로직은 전부 `@geowire/core`에 있다.
...
- `apps/api`와 `apps/mcp-server`를 **`apps/server` 하나로 통합** — 프로세스 하나가
  ...
  `npx @geowire/mcp`로도 단독 실행 가능. 배포·운영 단순화.
...
- 전송: stdio(`npx @geowire/mcp`) + Streamable HTTP(`apps/server` 내장).
...
```bash
npx geowire              # zero-config 서버 기동 (nominatim만으로)
npx geowire init         # 대화형 설정 마법사 (기획 §23)
npx geowire serve        # 설정 파일 기반 기동
npx geowire test         # 등록된 공급자 연결 검사 ("✓ Google Places connected")
npx geowire search "coffee near Gangnam"   # 터미널에서 바로 검색 (데모용 킬러 기능)
```
...
- [ ] `packages/cli` — `npx geowire`, `init`, `test`, `search`
...
1. ✅ **이름: GeoWire 확정** — 초안 이름 GeoRelay는 georelay.com(활성 GPS 추적 기업,
   ...
   GeoWire는 npm(`geowire`, `@geowire/*` 스코프 미사용 확인), GitHub org, geowire.dev
